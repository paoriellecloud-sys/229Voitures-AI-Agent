...
"""
    ...
    if not result:
        ...
        if _cmp_indices and session["context"].get("last_results"):
            _cmp_vehicles = session["context"]["last_results"]
            _cmp_pm = session["context"].get("price_max")
            _cmp_data = _bc(_cmp_vehicles, _cmp_indices, price_max=_cmp_pm)
