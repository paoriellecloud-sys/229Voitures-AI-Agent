...
def generate_rdv_form(vehicle: dict) -> str:
    ...
    vehicle_id = re.sub(r'[^a-zA-Z0-9]', '', str(
    ...
    return (
        ...
        f'<input type="text" placeholder="Votre nom complet" style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(250,199,117,0.20);border-radius:8px;padding:10px 12px;color:#f0f0f0;font-size:13px;outline:none;" id="rdv_nom_{vehicle_id}" onfocus="this.style.borderColor=\'rgba(250,199,117,0.6)\'" onblur="this.style.borderColor=\'rgba(250,199,117,0.20)\'"></div>'
        ...
        f'<input type="tel" placeholder="Votre num\u00e9ro de t\u00e9l\u00e9phone" style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(250,199,117,0.20);border-radius:8px;padding:10px 12px;color:#f0f0f0;font-size:13px;outline:none;" id="rdv_tel_{vehicle_id}" onfocus="this.style.borderColor=\'rgba(250,199,117,0.6)\'" onblur="this.style.borderColor=\'rgba(250,199,117,0.20)\'"></div>'
        ...
        f'<input type="email" placeholder="Votre adresse courriel" style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(250,199,117,0.20);border-radius:8px;padding:10px 12px;color:#f0f0f0;font-size:13px;outline:none;" id="rdv_email_{vehicle_id}" onfocus="this.style.borderColor=\'rgba(250,199,117,0.6)\'" onblur="this.style.borderColor=\'rgba(250,199,117,0.20)\'"></div>'
        ...
        f'<textarea placeholder="Pr\u00e9cisez vos disponibilit\u00e9s ou toute autre question..." rows="3" style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(250,199,117,0.20);border-radius:8px;padding:10px 12px;color:#f0f0f0;font-size:13px;outline:none;resize:none;font-family:inherit;" id="rdv_msg_{vehicle_id}" onfocus="this.style.borderColor=\'rgba(250,199,117,0.6)\'" onblur="this.style.borderColor=\'rgba(250,199,117,0.20)\'"></textarea></div>'
        f'<button onclick="[\'rdv_nom_{vehicle_id}\',\'rdv_tel_{vehicle_id}\',\'rdv_email_{vehicle_id}\',\'rdv_msg_{vehicle_id}\'].forEach(id=>{{const el=document.getElementById(id);if(el)el.dispatchEvent(new Event(\'input\'));}});const nom=document.getElementById(\'rdv_nom_{vehicle_id}\').value||document.getElementById(\'rdv_nom_{vehicle_id}\').defaultValue||\'\';const tel=document.getElementById(\'rdv_tel_{vehicle_id}\').value||document.getElementById(\'rdv_tel_{vehicle_id}\').defaultValue||\'\';const email=document.getElementById(\'rdv_email_{vehicle_id}\').value||document.getElementById(\'rdv_email_{vehicle_id}\').defaultValue||\'\';const msg=document.getElementById(\'rdv_msg_{vehicle_id}\').value||document.getElementById(\'rdv_msg_{vehicle_id}\').defaultValue||\'\';if(!nom.trim()||!tel.trim()||!email.trim()){{alert(\'Veuillez remplir les champs obligatoires (*)\');return;}}sendSuggestion(\'DEMANDE_RDV | V\u00e9hicule: {veh} | Concessionnaire: {dealer} | Nom: \'+nom.trim()+\' | T\u00e9l: \'+tel.trim()+\' | Courriel: \'+email.trim()+(msg.trim()?\' | Message: \'+msg.trim():\'\'));" style="background:#FAC775;color:#1a1a1a;border:none;border-radius:8px;padding:12px 18px;font-size:13px;font-weight:700;cursor:pointer;width:100%;" onmouseover="this.style.background=\'#f0b84d\'" onmouseout="this.style.background=\'#FAC775\'">Envoyer ma demande &#8599;</button>'
