...
def generate_rdv_form(vehicle: dict) -> str:
    ...
    return (
        ...
        f'<button onclick="[\'rdv_nom\',\'rdv_tel\',\'rdv_email\',\'rdv_msg\'].forEach(id=>{{const el=document.getElementById(id);if(el)el.dispatchEvent(new Event(\'input\'));}});const nom=document.getElementById(\'rdv_nom\').value||document.getElementById(\'rdv_nom\').defaultValue||\'\';const tel=document.getElementById(\'rdv_tel\').value||document.getElementById(\'rdv_tel\').defaultValue||\'\';const email=document.getElementById(\'rdv_email\').value||document.getElementById(\'rdv_email\').defaultValue||\'\';const msg=document.getElementById(\'rdv_msg\').value||document.getElementById(\'rdv_msg\').defaultValue||\'\';if(!nom.trim()||!tel.trim()||!email.trim()){{alert(\'Veuillez remplir les champs obligatoires (*)\');return;}}sendSuggestion(\'DEMANDE_RDV | V\u00e9hicule: {veh} | Concessionnaire: {dealer} | Nom: \'+nom.trim()+\' | T\u00e9l: \'+tel.trim()+\' | Courriel: \'+email.trim()+(msg.trim()?\' | Message: \'+msg.trim():\'\'));" style="background:#FAC775;color:#1a1a1a;border:none;border-radius:8px;padding:12px 18px;font-size:13px;font-weight:700;cursor:pointer;width:100%;" onmouseover="this.style.background=\'#f0b84d\'" onmouseout="this.style.background=\'#FAC775\'">Envoyer ma demande &#8599;</button>'
